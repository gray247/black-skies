...
const applyVisualStableAttrs = (element) => {
    if (!element) {
        return;
    }
...
const ensureVisualStableAttrsWithRetry = () => {
...
const applyForceStateAttributes = () => {
...
const ensureForceStateAttrsWithRetry = () => {
    if (!harnessHooksEnabled) {
        return;
    }
    if (typeof document === 'undefined') {
...
if (typeof window !== 'undefined' && harnessHooksEnabled) {
    const setHarnessFlag = (flag, enabled) => {
        if (typeof document === 'undefined') {
            return false;
        }
        const root = document.documentElement;
    ...
    const applyHarnessFlags = () => {
        const activeFlowApplied = setHarnessFlag('testActiveFlow', process.env.PLAYWRIGHT === '1');
        const stableDockApplied = setHarnessFlag('testStableDock', process.env.BLACKSKIES_STABLE_DOCK === '1');
        const stableHomeApplied = setHarnessFlag('testStableHome', process.env.BLACKSKIES_STABLE_HOME === '1');
        const visualStableApplied = setHarnessFlag('testVisualStable', process.env.BLACKSKIES_VISUAL_STABLE === '1');
        return activeFlowApplied || stableDockApplied || stableHomeApplied || visualStableApplied;
    };
    const ensureHarnessFlags = () => {
...
async function makeServiceCall(path, method, body) {
    ...
    if (!port) {
        console.warn('[preload] makeServiceCall missing service port for', path, {
            env: {
...
function serializeDraftRewriteRequest({ projectId, draftId, unitId, unit, instructions, newText, }) {
...
exports.serviceApi = {
    ...
    rewriteDraft: (request) => makeServiceCall('draft/rewrite', 'POST', serializeDraftRewriteRequest(request)),
    ...
    restoreFromZip: (request) => makeServiceCall('restore', 'POST', {
    ...
    listBackups: (request) => {
        ...
        return makeServiceCall(`backups?${params.toString()}`, 'GET');
    },
    restoreBackup: (request) => makeServiceCall('backups/restore', 'POST', {
...
const servicesBridge = {
    ...
    rewriteDraft: exports.serviceApi.rewriteDraft,
