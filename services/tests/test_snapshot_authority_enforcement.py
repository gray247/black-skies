...
def _workspace_temp_dir() -> Path:
    root = Path.cwd() / ".tmp" / f"blackskies-snapshot-authority-{uuid4().hex}"
...
def test_accept_snapshot_path_uses_history_authority() -> None:
...
def test_recovery_status_does_not_source_last_snapshot_from_dot_snapshots() -> None:
    tmp_path = _workspace_temp_dir()
    try:
...
def test_recovery_restore_rejects_snapshot_ids_present_only_in_dot_snapshots() -> None:
    tmp_path = _workspace_temp_dir()
    try:
