...
def test_memory_accept_race_resolution(tmp_path: Path) -> None:
    ...
    case = next(item for item in project["cases"] if item["case_id"] == "beta_same_scene_parallel_accept")
    ...
    packet_files = list((project_root / ".blackskies" / "memory" / "packets" / "rewrite").glob("*.json"))
    ...
    packet_files_after = list((project_root / ".blackskies" / "memory" / "packets" / "rewrite").glob("*.json"))
