...
import blackskies.services.memory_lab.environment as environment_module
...
from blackskies.services.memory_lab.environment import detect_environment_tier
...
@contextmanager
def _lock_ctx(lock_state: LockState):
    yield lock_state
...
def test_detect_environment_tier_supported(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        environment_module,
    ...
    assert tier.tier == "supported_deterministic"
    assert tier.is_supported_deterministic is True


def test_detect_environment_tier_best_effort(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        environment_module,
        "acquire_project_lock",
    ...
    assert tier.tier == "best_effort"
    assert tier.is_supported_deterministic is False


def test_orchestrator_reports_environment_tier(tmp_path: Path, monkeypatch) -> None:
    ...
    assert diagnostics.environment_tier == "supported_deterministic"
