...
// HARNESS_ONLY:
// Reason: validates dock workspace shell behavior under harness init scripts.
// Owner: app/tests/e2e/dock-workspace.spec.ts
// Retire when: equivalent dock assertions are covered in real-service truth workflows.
    ...
    ({ project }) => {
      ...
      const layoutBridge = {
        ...
        async closeFloatingPane(request: { paneId: string }) {
          layoutState.floatingPanes = layoutState.floatingPanes.filter(
            (entry) => entry.id !== request.paneId,
          );
          return;
        },
      };
...
test.describe('Dock workspace interactions', () => {
  test('supports drag, float, and focus controls', async ({ page }) => {
    ...
    await expect(
      page.locator('.dock-pane__toolbar').first().getByTitle('Close this pane.'),
    ).toBeVisible();
    ...
    await page.evaluate(() => {
      ...
      (window as typeof window & { __stableDockHandleReady?: boolean }).__stableDockHandleReady =
        true;
    });
    await page.waitForFunction(
      () =>
        (window as typeof window & { __stableDockHandleReady?: boolean })
          .__stableDockHandleReady === true,
    );
    ...
    await expect
      .poll(async () =>
        page.evaluate(
          () => (document.activeElement as HTMLElement | null)?.dataset?.paneId ?? null,
        ),
      )
      .toBe('draftPreview');
  });
