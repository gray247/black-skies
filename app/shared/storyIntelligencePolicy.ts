...
export function isStoryPositionRefV1(
  value: unknown,
  projectId: string,
): value is StoryPositionRefV1 {
  if (
    !isRecord(value) ||
    !hasExactKeys(
      value,
    ...
    return false;
  const hasSelectionCoordinates =
    value.selectionStart !== undefined || value.selectionEnd !== undefined;
  const hasValidSelectionCoordinates =
    !hasSelectionCoordinates ||
    (isInteger(value.selectionStart) &&
      value.selectionStart >= 0 &&
      isInteger(value.selectionEnd) &&
      value.selectionEnd >= value.selectionStart &&
      isSha256(value.selectionFingerprint) &&
  ...
  return (
    ...
    hasValidSelectionCoordinates &&
    (value.bodySha256 === undefined || isSha256(value.bodySha256)) &&
    (value.orderIndex === undefined || isInteger(value.orderIndex)) &&
    (value.orderBasis === undefined ||
      isOneOf(value.orderBasis, ['manuscript', 'story-world', 'planning', 'reveal', 'projection']))
  );
}

export function isStoryIntelligenceProvenanceV1(
  value: unknown,
): value is StoryIntelligenceProvenanceV1 {
  if (
    !isRecord(value) ||
    !hasExactKeys(value, [
      'sourceOwner',
    ...
    return false;
  return (
    isBoundedString(value.sourceOwner, 160) &&
...
function isPolicy(value: unknown): value is StoryIntelligenceAnalysisPolicyV1 {
  if (
    ...
    return false;
  return (
    value.schemaVersion === STORY_INTELLIGENCE_POLICY_SCHEMA_VERSION &&
    ...
    (value.allowedSourceClasses as unknown[]).every((item) => isOneOf(item, SOURCE_CLASSES_V1)) &&
    Array.isArray(value.excludedSourceClasses) &&
    (value.excludedSourceClasses as unknown[]).every((item) => isOneOf(item, SOURCE_CLASSES_V1)) &&
...
function isAuthorRecord(
  value: unknown,
  projectId: string,
): value is StoryIntelligenceAuthorRecordV1 {
  if (
    !isRecord(value) ||
    !hasExactKeys(
      value,
  ...
  return (
    ...
    (!emotionRecord ||
      (isOneOf(value.emotionLane, ['planned', 'observed', 'reader-effect-optional']) &&
        isOneOf(value.emotionIntensity, [
          'very-low',
          'low',
          'medium',
          'high',
    ...
    (!timelineRecord ||
      (isInteger(value.timelineWorldOrder) &&
        isOneOf(value.timelineTemporalState, [
          'certain',
          'uncertain',
          'disputed',
          'simultaneous',
    ...
    (!pacingRecord ||
      isOneOf(value.pacingTempo, ['very-slow', 'slow', 'steady', 'fast', 'very-fast'])) &&
    (!pressureRecord ||
      (isOneOf(value.evidenceClass, ['planned', 'observed']) &&
    ...
    (value.emotionLane === undefined ||
      isOneOf(value.emotionLane, ['planned', 'observed', 'reader-effect-optional'])) &&
    (value.emotionIntensity === undefined ||
      isOneOf(value.emotionIntensity, [
        'very-low',
        'low',
        'medium',
    ...
    (value.timelineTemporalState === undefined ||
      isOneOf(value.timelineTemporalState, [
        'certain',
        'uncertain',
        'disputed',
    ...
    (value.pacingTempo === undefined ||
      isOneOf(value.pacingTempo, ['very-slow', 'slow', 'steady', 'fast', 'very-fast'])) &&
    (value.pressureDimension === undefined ||
      isOneOf(value.pressureDimension, ['urgency', 'consequence', 'constraint', 'conflict'])) &&
    (value.pressureBand === undefined ||
      isOneOf(value.pressureBand, ['none', 'low', 'medium', 'high', 'very-high', 'unknown'])) &&
    (value.currentness === undefined || isOneOf(value.currentness, CURRENTNESS_VALUES_V1)) &&
    Array.isArray(value.positionRefs) &&
    value.positionRefs.every((ref) => isStoryPositionRefV1(ref, projectId)) &&
    isStoryIntelligenceProvenanceV1(value.provenance) &&
...
function isDurableSignal(value: unknown, projectId: string): value is DurableSignalV1 {
  if (
    ...
    !hasExactKeys(
      ...
      ['sourceFindingId', 'disposition'],
    )
  )
    return false;
  return (
    value.schemaVersion === STORY_INTELLIGENCE_SCHEMA_VERSION &&
    isBoundedString(value.signalId, 160) &&
    value.projectId === projectId &&
    isOptionalBoundedString(value.sourceFindingId, 160) &&
    Array.isArray(value.positionRefs) &&
    value.positionRefs.every((ref) => isStoryPositionRefV1(ref, projectId)) &&
    isBoundedString(value.sourceOwner, 160) &&
    isOneOf(value.evidenceClass, EVIDENCE_CLASSES_V1) &&
    isOneOf(value.impact, SIGNAL_IMPACTS_V1) &&
    isOneOf(value.confidenceBand, CONFIDENCE_BANDS_V1) &&
    isOneOf(value.currentness, CURRENTNESS_VALUES_V1) &&
    value.lifecycle !== 'candidate' &&
    isOneOf(value.lifecycle, SIGNAL_LIFECYCLES_V1) &&
    isBoundedString(value.summary, 800) &&
    isBoundedString(value.evidenceSummary, 1200) &&
    isStoryIntelligenceProvenanceV1(value.provenance) &&
    (value.disposition === undefined ||
      isOneOf(value.disposition, [
        'dismissed',
        'suppressed',
        'expired',
        'converted',
...
function isDisposition(value: unknown, projectId: string): boolean {
  ...
  return (
    value.projectId === projectId &&
    isBoundedString(value.dispositionId, 160) &&
    isBoundedString(value.signalId, 160) &&
    isOneOf(value.lifecycle, [
      'dismissed',
      'suppressed',
      'expired',
      'converted',
    ...
    isOneOf(value.actor, ['author', 'system']) &&
    isIsoDate(value.createdAt)
  );
}

function isHistoryEvent(
  value: unknown,
  projectId: string,
): value is StoryIntelligenceHistoryEventV1 {
  if (
    !isRecord(value) ||
    !hasExactKeys(
      value,
      ['eventId', 'projectId', 'eventType', 'subjectId', 'actor', 'createdAt'],
      [
        'sourceRevision',
        'lifecycleBefore',
        'lifecycleAfter',
  ...
  return (
    value.projectId === projectId &&
    isBoundedString(value.eventId, 160) &&
    ...
    isOptionalBoundedString(value.provenanceRef, 160) &&
    isIsoDate(value.createdAt)
  );
}

export function defaultStoryIntelligencePolicy(
  now = new Date(),
...
export function createDefaultStoryIntelligenceDocument(
  projectId: string,
  now = new Date(),
...
export function validateStoryIntelligenceDocument(
  value: unknown,
  projectId: string,
): StoryIntelligenceDocumentV1 {
  const issues: string[] = [];
  ...
  if (value.schemaVersion !== STORY_INTELLIGENCE_SCHEMA_VERSION)
  ...
  if (
    ...
    !hasExactKeys(value.settings, ['signalPosture', 'projectPosture', 'analysisPolicy']) ||
    !isOneOf(value.settings.signalPosture, SIGNAL_POSTURES_V1) ||
    !isOneOf(value.settings.projectPosture, PROJECT_POSTURES_V1) ||
    !isPolicy(value.settings.analysisPolicy)
  )
    issues.push('settings are invalid');
  if (
    !Array.isArray(value.unitPolicies) ||
    !value.unitPolicies.every((item) => {
      if (
        !isRecord(item) ||
        !hasExactKeys(item, ['unitId', 'enabled', 'updatedAt'], ['signalPosture', 'projectPosture'])
      )
        return false;
      return (
        isBoundedString(item.unitId, 160) &&
        typeof item.enabled === 'boolean' &&
        (item.signalPosture === undefined || isOneOf(item.signalPosture, SIGNAL_POSTURES_V1)) &&
        (item.projectPosture === undefined || isOneOf(item.projectPosture, PROJECT_POSTURES_V1)) &&
  ...
  if (
    !Array.isArray(value.authorRecords) ||
    !value.authorRecords.every((item) => isAuthorRecord(item, projectId))
  )
    issues.push('author records are invalid');
  if (
    !Array.isArray(value.durableSignals) ||
    !value.durableSignals.every((item) => isDurableSignal(item, projectId))
  )
    issues.push('durable signals are invalid');
  if (
    !Array.isArray(value.dispositions) ||
    !value.dispositions.every((item) => isDisposition(item, projectId))
  )
    issues.push('dispositions are invalid');
  if (
    !Array.isArray(value.history) ||
    value.history.length > STORY_INTELLIGENCE_HISTORY_LIMIT ||
    !value.history.every((item) => isHistoryEvent(item, projectId))
  )
    issues.push('history is invalid or exceeds the bounded limit');
...
export function canTransitionSignalLifecycle(
  from: SignalLifecycleV1,
  to: SignalLifecycleV1,
): boolean {
  if (from === 'candidate') return to === 'reviewed' || to === 'dismissed' || to === 'expired';
  if (from === 'reviewed')
    return ['accepted', 'dismissed', 'suppressed', 'expired', 'converted', 'resolved'].includes(to);
  if (from === 'accepted')
    return ['suppressed', 'converted', 'resolved', 'expired', 'superseded'].includes(to);
...
export function deriveStoryPositionCurrentness(
  reference: Pick<StoryPositionRefV1, 'sourceRevision' | 'sourceFingerprint' | 'bodySha256'>,
  current: {
  ...
  if (reference.bodySha256 !== undefined && current.bodySha256 !== undefined) {
    return reference.bodySha256 === current.bodySha256 ? 'current' : 'stale';
  }
  if (
    current.sourceRevision !== reference.sourceRevision ||
    current.sourceFingerprint !== reference.sourceFingerprint
  )
    return 'stale';
  return 'current';
}
...
export function checkStoryIntelligencePermission(
  ...
  if (operation === 'display-metadata' && PROTECTED_SOURCE_CLASSES.has(sourceClass)) {
    return {
      ...
      operation,
      metadataOnly: true,
      reason: 'protected-metadata-only',
    };
  }
...
export function isDurableSignalLifecycle(
  value: SignalLifecycleV1,
): value is DurableSignalV1['lifecycle'] {
