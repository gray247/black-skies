...
export default function ContinuityReview({
  ...
  onAction,
  onWorkOnThis,
}: ContinuityReviewProps): JSX.Element {
  const titleId = useId();
  const descriptionId = useId();
  return (
    <section
      className="continuity-review"
      aria-labelledby={titleId}
      aria-describedby={descriptionId}
      data-testid="continuity-review"
    >
      <header className="continuity-review__header">
        <div>
          <h2 id={titleId}>Continuity review</h2>
          <p id={descriptionId}>
            Advisory structured findings. Review the cited source before taking any owner-governed
            action.
          </p>
        </div>
      ...
      {findings.length === 0 ? (
        <p className="continuity-review__empty" data-testid="continuity-review-empty">
          No current structured continuity findings are available.
        </p>
      ) : (
              ...
              {findings.map((finding) => (
                <tr key={finding.findingId} data-testid="continuity-finding">
                  ...
                  <td>
                    ...
                    <span className="continuity-review__evidence">{finding.evidenceSummary}</span>
                    <span className="continuity-review__refs">
                      Refs:{' '}
                  ...
                  <td>
                    <div
                      className="continuity-review__actions"
                      aria-label={`Actions for ${finding.summary}`}
                    >
                      ...
                      {finding.currentness === 'current' && finding.positionRefs[0] ? (
                        <button
                          type="button"
                          onClick={() => {
                            ...
                            void buildProgram7SourceEnvelope({
                              ...
                              sourceClass: finding.provenance.protectionClass,
                              ...
                              evidenceSummary:
                                finding.provenance.protectionClass === 'included' ||
                                finding.provenance.protectionClass === 'deterministic-only' ||
                                finding.provenance.protectionClass === 'local-only'
