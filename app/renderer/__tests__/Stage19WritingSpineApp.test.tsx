...
function createStoryIntelligenceBridge(): StoryIntelligenceBridge {
  ...
  const document: StoryIntelligenceDocumentV1 = {
    ...base,
    authorRecords: [{
      recordId: 'emotion-source-return',
      projectId: 'proj_a',
      unitId: 'unit_a',
...
function createLivingOutlineBridge(initialItems: LivingOutlineDocumentV1['items'] = []) {
  ...
  const bridge: LivingOutlineBridge = {
    ...
    updateItem: vi.fn(async (request) => update(current.document.items.map((item) => item.id === request.itemId
      ? { ...item, label: request.label, kind: request.kind, state: request.state, body: request.body }
      : item))),
...
type WritingRailLabel = 'Project Tools' | 'Story' | 'Review' | 'Writing Session';
...
async function addStoryPoint(user: ReturnType<typeof userEvent.setup>): Promise<void> {
  await user.click(screen.getByRole('button', { name: 'Add story content' }));
...
describe('Stage19WritingSpineApp', () => {
  ...
  it('uses writer-facing Story language while preserving the existing safe storage calls', async () => {
    ...
    const addToStory = screen.getByRole('button', { name: 'Add story content' });
    expect(addToStory).toHaveAttribute('title', 'Add a Unit or Note');

    await addStoryPoint(user);
  ...
  it('uses the compact plus chooser and selected-item minus confirmation', async () => {
    const project = createBridge(snapshot('writing'));
    const outline = createLivingOutlineBridge([{
  ...
  it('dismisses the add chooser without creating a placeholder', async () => {
    const project = createBridge(snapshot('writing'));
    const outline = createLivingOutlineBridge();
    const user = userEvent.setup();
    render(<Stage19WritingSpineApp windowRole="writing" bridge={project.bridge} livingOutlineBridge={outline.bridge} />);
    ...
    expect(outline.bridge.createItem).not.toHaveBeenCalled();
  ...
  it('supports quiet derived context and deliberate multi-selection without touching prose', async () => {
    const project = createBridge(snapshot('writing'));
    const outline = createLivingOutlineBridge([{
      id: 'outline-a', label: 'Opening note', kind: 'fragment', state: 'planned', manuscriptUnitId: 'unit_a',
      createdAt: '2026-08-09T12:00:00.000Z', updatedAt: '2026-08-09T12:00:00.000Z',
    }]);
    const user = userEvent.setup();
    render(<Stage19WritingSpineApp windowRole="writing" bridge={project.bridge} livingOutlineBridge={outline.bridge} />);
    ...
    await user.click(screen.getByRole('button', { name: 'Story rail help' }));
    expect(screen.getByRole('note', { name: 'Story rail help' })).toHaveTextContent(/Unit.*manuscript.*Note.*planning.*unlinked/i);
    await user.click(screen.getByRole('button', { name: 'Select multiple' }));
    expect(screen.getByRole('checkbox', { name: 'Select Unit First Unit' })).toBeVisible();
    expect(screen.getByRole('checkbox', { name: 'Select Note Opening note' })).toBeVisible();
    ...
    expect(screen.getByRole('button', { name: 'Open Note Opening note for First Unit' }).closest('.stage19-note-marker-cluster')).toHaveClass('is-multi-selected');
    expect(project.bridge.saveUnit).not.toHaveBeenCalled();
    expect(project.bridge.reorderUnits).not.toHaveBeenCalled();

    await user.click(screen.getByRole('button', { name: 'Done selecting' }));
    expect(screen.queryByRole('checkbox', { name: 'Select Unit First Unit' })).not.toBeInTheDocument();
  });

  it('shows a single Note title and supports F2 and double-click rename without shifting focus on ordinary open', async () => {
    const project = createBridge(snapshot('writing'));
    const outline = createLivingOutlineBridge([{
      id: 'outline-a', label: 'Opening note', kind: 'fragment', state: 'planned', manuscriptUnitId: 'unit_a',
      createdAt: '2026-08-09T12:00:00.000Z', updatedAt: '2026-08-09T12:00:00.000Z',
    ...
    const marker = screen.getByRole('button', { name: 'Open Note Opening note for First Unit' });
    expect(marker).toHaveTextContent('Opening note');
    marker.focus();
    fireEvent.keyDown(marker, { key: 'F2' });

    const title = await screen.findByRole('textbox', { name: 'Title for Opening note' });
    expect(title).toHaveFocus();
    await user.clear(title);
    await user.type(title, 'Revised opening note');
    await user.click(screen.getByRole('button', { name: 'Save note and close' }));

    const renamedMarker = await screen.findByRole('button', { name: 'Open Note Revised opening note for First Unit' });
    expect(renamedMarker).toHaveTextContent('Revised opening note');
    expect(outline.bridge.updateItem).toHaveBeenCalledWith(expect.objectContaining({
      itemId: 'outline-a',
      label: 'Revised opening note',
    }));

    await user.click(renamedMarker);
    expect(screen.getByRole('textbox', { name: 'Title for Revised opening note' })).not.toHaveFocus();
    const detail = screen.getByRole('region', { name: 'More options for Revised opening note' });
    await user.click(within(detail).getByRole('button', { name: 'Close note editor; discard unsaved edits' }));
    await user.dblClick(screen.getByRole('button', { name: 'Open Note Revised opening note for First Unit' }));
    expect(await screen.findByRole('textbox', { name: 'Title for Revised opening note' })).toHaveFocus();
  });
  ...
  it('returns a detached Emotion Graph point to Writing Studio before selecting its linked unit', async () => {
    const units = [
      { id: 'unit_a', title: 'First Unit', order: 1, body: 'Alpha body' },
      { id: 'unit_b', title: 'Second Unit', order: 2, body: 'Beta body' },
    ];
    ...
    await user.click(screen.getByRole('button', { name: 'Story Knowledge' }));
    await user.click(screen.getByRole('button', { name: 'Emotion' }));
    await user.click(await screen.findByRole('button', { name: /planned: guarded hope/i }));

    await waitFor(() => expect(command.bridge.selectUnit).toHaveBeenCalledWith(
      expect.objectContaining({ unitId: 'unit_a' }),
    ));
    expect(surfaces.activateSurface).toHaveBeenCalledWith(expect.objectContaining({
      targetSurface: 'writing',
      placement: 'current-window',
    }));
    expect(vi.mocked(surfaces.activateSurface).mock.invocationCallOrder[0])
      .toBeLessThan(vi.mocked(command.bridge.selectUnit).mock.invocationCallOrder[0]!);
    expect(surfaces.current).toMatchObject({
      primarySurface: 'writing',
      commandPlacement: 'secondary-window',
      secondaryStatus: 'open',
  ...
  it('centers Writing Studio when a detached Command Center selection is broadcast', async () => {
    const scrollIntoView = vi.fn();
    const originalScrollIntoView = HTMLElement.prototype.scrollIntoView;
    Object.defineProperty(HTMLElement.prototype, 'scrollIntoView', { configurable: true, value: scrollIntoView });
    try {
      const writing = createBridge(snapshot('writing', { activeUnitId: 'unit_b' }));
      const surfaces = createSurfaceBridge(snapshot('command', { activeUnitId: 'unit_b' }), {
        initialPlacement: 'secondary-window',
      });
      render(
        <Stage19WritingSpineApp
      ...
      await screen.findByRole('textbox', { name: 'Manuscript editor: Untitled' });
      scrollIntoView.mockClear();
      act(() => writing.emit(snapshot('writing', { activeUnitId: 'unit_a', revision: 2 })));

      await waitFor(() => expect(scrollIntoView).toHaveBeenCalledWith({
        behavior: 'smooth',
        block: 'center',
      }));
      expect(document.getElementById('stage19-manuscript-unit-unit_a')).toHaveAttribute(
        'data-manuscript-unit-anchor',
        'true',
      );
    } finally {
  ...
  it('routes a local Companion orientation request through the persistent Companion bar without changing writing', async () => {
    ...
    expect(screen.getByRole('region', { name: 'Companion orientation result' })).toHaveTextContent('No Note is placed with the current writing');
  ...
  it('renders Writing Studio as the only prose-editing and structural authority', async () => {
    ...
    await openWritingRail('Story');
    expect(screen.getByRole('button', { name: 'Add story content' })).toBeVisible();
    expect(screen.getByRole('button', { name: 'Delete selected story item' })).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'Unit' })).not.toBeInTheDocument();
  ...
  it('keeps a directly writable manuscript at the center while every support family starts closed', async () => {
    ...
    for (const label of ['Project Tools', 'Story', 'Review', 'Writing Session'] as const) {
    ...
    expect(screen.queryByLabelText('Writing Session')).not.toBeInTheDocument();
  });
  ...
  it('blocks editing while showing full prior-session prose and sends exact recovery decisions', async () => {
    ...
    await openWritingRail('Story');
    await userEvent.click(screen.getByRole('button', { name: 'Add story content' }));
    expect(screen.getByRole('button', { name: 'Unit' })).toBeDisabled();
    await act(async () => {
  ...
  it('routes contextual create, inline rename, and advanced delete through Writing-only bindings', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'More options for Unit First Unit' }));
    await act(async () => {
      await user.click(screen.getByRole('button', { name: 'Delete selected story item' }));
    });
  ...
  it('jumps the continuous manuscript to the selected rail unit', async () => {
    const scrollIntoView = vi.fn();
    const originalScrollIntoView = HTMLElement.prototype.scrollIntoView;
    Object.defineProperty(HTMLElement.prototype, 'scrollIntoView', { configurable: true, value: scrollIntoView });
    try {
      ...
      await user.click(screen.getByRole('button', { name: '02 Untitled' }));
      ...
      expect(document.getElementById('stage19-manuscript-unit-unit_b')).toHaveAttribute('data-manuscript-unit-anchor', 'true');
      expect(screen.getByRole('textbox', { name: 'Manuscript editor: Untitled' })).toHaveValue('Beta body');
    } finally {
      if (originalScrollIntoView) {
        Object.defineProperty(HTMLElement.prototype, 'scrollIntoView', { configurable: true, value: originalScrollIntoView });
      } else {
  ...
  it('supports outline-first planning with quiet gaps and no manufactured manuscript unit', async () => {
    ...
    expect(screen.getByText(/No Unit selected; new Notes remain unlinked/i)).toBeVisible();
    ...
    await addStoryPoint(user);
    const title = await screen.findByRole('textbox', { name: 'Title for New note' });
    await user.clear(title);
    ...
    await user.click(screen.getByRole('button', { name: 'Save note and close' }));
    await user.click(screen.getByRole('button', { name: 'Open Note What happens between the signal and arrival? not placed yet' }));
    ...
    expect(screen.queryByRole('region', { name: 'More options for What happens between the signal and arrival?' })).not.toBeInTheDocument();
    ...
    expect(await screen.findByRole('button', { name: 'Open Note What happens between the signal and arrival? not placed yet' })).toHaveTextContent('What happens between the signal and arrival?');
  ...
  it('saves a prose-free span anchor and returns a Story Rail point to its exact passage', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: /Open Note .* for First Unit/ }));
  ...
  it('locates linked writing from either side and previews planning movement without reordering manuscript units', async () => {
    ...
    await openWritingRail('Story');
    const opening = await screen.findByRole('button', { name: 'Open Note Opening for First Unit' });
    const openingCluster = opening.closest('.stage19-note-marker-cluster');
    expect(openingCluster).toBeVisible();
    const turn = screen.getByRole('button', { name: 'Open Note Uncertain turn for Untitled' });
  ...
  it('reorders the planning sidecar from the keyboard without moving accepted manuscript units', async () => {
    ...
    await openWritingRail('Story');
    const turnPosition = await screen.findByRole('button', { name: 'Open Note Turn for Untitled' });
  ...
  it('places a story point under another written section by direct drop without reordering prose', async () => {
    ...
    await openWritingRail('Story');
    const storyPoint = await screen.findByRole('button', { name: 'Open Note Opening for First Unit' });
