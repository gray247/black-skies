...
const panes: Partial<Record<LayoutPaneId, JSX.Element>> = {
  storyInsights: <CounterPane id="storyInsights" />,
  corkboard: <CounterPane id="corkboard" />,
  relationshipGraph: <CounterPane id="relationshipGraph" />,
  outline: <div>Wizard</div>,
  'draftPreview': <div>Draft</div>,
  timeline: <div>History</div>,
...
function TabHarness(): JSX.Element {
  const [active, setActive] = useState<LayoutPaneId>('storyInsights');
  return (
    <div>
      <button onClick={() => setActive('storyInsights')}>Story Insights</button>
      <button onClick={() => setActive('corkboard')}>Corkboard</button>
      <button onClick={() => setActive('relationshipGraph')}>Relationships</button>
      {panes[active]}
    </div>
  );
}
...
describe('Approximate memory leak safeguards', () => {
  ...
  it('does not leak mounts when switching tabs repeatedly', async () => {
    ...
    render(<TabHarness />);
    const storyInsightsBtn = screen.getByText('Story Insights');
    const corkboardBtn = screen.getByText('Corkboard');
    const relationshipGraphBtn = screen.getByText('Relationships');

    for (let i = 0; i < 10; i += 1) {
      await user.click(corkboardBtn);
      await user.click(relationshipGraphBtn);
      await user.click(storyInsightsBtn);
    }

    expect(mountCounts.storyInsights).toBeLessThanOrEqual(2);
    expect(mountCounts.corkboard).toBeLessThanOrEqual(2);
    expect(mountCounts.relationshipGraph).toBeLessThanOrEqual(2);
  });
  ...
  it('does not accumulate listeners on project reload', () => {
    ...
    const { unmount } = render(<ListenerHarness />);
    unmount();
