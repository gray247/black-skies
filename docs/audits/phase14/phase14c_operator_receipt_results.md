...
- Latest operator rerun sequence after commit `589d7e88` produced a mixed early result and then stabilized after a full Black Skies restart. One failure was still observed in the pre-restart lane, but the operator later closed and restarted Black Skies and then observed two successful restore-latest reruns. The successful runs showed `Restore copy created`, and restored sibling copies exist under `C:\Dev\black-skies\sample_project`.
- Mixed-result reconciliation: restore-latest is no longer fully blocked as an authority contradiction because the operator has now observed repeated real sibling-copy materialization through the live UI after restart. Reliability remains a bounded concern because earlier attempts in the same overall verification period still failed, but the post-restart live path is now proven to work more than once.
- Current 14C posture recommendation: treat restore-latest as `Pass with reliability exception` for Phase 14C reconciliation and closure-readiness, not as reliability closure. No further restore-latest reruns are recommended for Phase 14C because the authority question has been answered and repeated attempts mainly create more restored sibling copies. Intermittent timeout/materialization variance should remain deferred to Phase 15 backup/restore hardening rather than re-open Phase 14C as a full hard blocker.
...
- Actual result: Earlier May 15-16 attempts exposed four staged failures: payload mismatch, lookup mismatch, `45,000ms` timeout, and then `120,000ms` timeout despite background copy creation. After the restore-only timeout was raised again in commit `589d7e88`, the operator first observed a mixed rerun with one failure and one success. The operator then closed and restarted Black Skies and observed two more successful restore-latest reruns, each with visible `Restore copy created` UI feedback and a new sibling restored copy under `C:\Dev\black-skies\sample_project`.
...
- Any receipts deferred to Phase 15: backup timeout remains deferred as an operational-risk item, pending reproduction; broader restore hardening remains Phase 15 scope beyond the payload-contract, lookup-alignment, and narrow restore-timeout fixes; intermittent restore-latest timeout/reliability variance is also deferred there rather than treated as a new Phase 14 implementation lane; and the `Writing tools offline` / `checking` state remains a startup-race observation pending future isolation rather than a proven restore-causal defect.
...
- Whether the packet is complete: sufficient for a `Pass with reliability exception` and closure-ready Phase 14C posture. Restore-latest is no longer a full 14C blocker because live success is now operator-observed and was later repeated twice after restart, but reliability is not closed and remains deferred to Phase 15.
