...
- **API/Data Model Schema Validation:** Harden every endpoint contract (DraftUnit, outline, revisions, critique, exports) with explicit schema validation, rate/concurrency guidance, error codes, guardrails for large payloads, and consistent metadata fields aligned with `docs/specs/data_model.md`.
...
### Phase 9 Index
- [`docs/phases/phase9_11_testplan.md`](./phase9_11_testplan.md) – Regression suites & telemetry assertions that prove Phase 9/11 behaviors across Critique, Recovery, Export, and analytics flows.
- [`docs/phases/phase9_companion_loop.md`](./phase9_companion_loop.md) – Companion automation plan that wires Overseer, Critique, and Rewrite services into the Critique Pane.
- [`docs/specs/analytics_service_spec.md`](../specs/analytics_service_spec.md) – Canonical analytics metric + cache service spec that feeds the dashboards and health overlays.
- [`docs/phases/dashboard_initiatives.md`](./dashboard_initiatives.md) – Dashboard deliverables, dependencies, and phase tracking for Project Health, Outline validation, and multi-project launchers.
- [`docs/specs/performance_telemetry_policy.md`](../specs/performance_telemetry_policy.md) – Telemetry targets, collection rules, and red lines for analytics/critic throughput.
- [`docs/ops/support_playbook.md`](../ops/support_playbook.md) – Operational response flows for analytics dashboards, backup verification health, and incident escalation.
- [`phase_log.md`](../phase_log.md) – Timeline of locks/status changes that confirm the chartered milestones.
