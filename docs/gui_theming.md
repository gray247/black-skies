# docs/gui_theming.md — DRAFT

## Purpose
Separate layout (see `docs/gui_layouts.md`) from theming so visual treatments can evolve without disrupting pane placement or keyboard navigation.
...
## Constraints
- Themes must leave pane layout untouched; no theme changes should reposition Outline/Writing/Feedback columns (those remain defined in `docs/gui_layouts.md`).
...
- The “GUI overhaul” (future presets like Spark Pad or Visuals Layer) is realized by composing new layouts/presets in `docs/gui_layouts.md` plus these theme changes rather than rewriting the whole architecture.
