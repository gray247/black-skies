...
4. Electron spawns the FastAPI service automatically; the status pill should transition from “Checking services” to “Services online” once `/api/v1/healthz` responds (legacy `/healthz` remains available with deprecation headers).
