...
2. **Environment** — Open `.env` and verify the entries from the offline prerequisites (`BLACK_SKIES_BLACK_SKIES_MODE=offline`, `BLACKSKIES_PROJECT_BASE_DIR=…`). The loader now emits the `BLACK_SKIES_BLACK_SKIES_MODE` key by default, but legacy `.env` files with `BLACK_SKIES_MODE` still work and log a reminder to rename it.【F:services/src/blackskies/services/settings.py†L1-L119】【F:services/src/blackskies/services/config.py†L14-L74】
...
| Services attempt external providers | Set `BLACK_SKIES_BLACK_SKIES_MODE=offline` (default) and a valid `BLACKSKIES_PROJECT_BASE_DIR` in `.env`; refer to the service configuration notes in the README. Legacy `.env` files using `BLACK_SKIES_MODE` still load but log a rename warning.【F:services/src/blackskies/services/settings.py†L1-L119】【F:services/src/blackskies/services/config.py†L14-L74】【F:README.md†L32-L55】 |
