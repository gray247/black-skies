...
- **Insights split**
  ...
  - "Run all" executes only the available local insights while offline, holds the model-backed work in-session, and displays "3 local ran - 5 will resume when online."
...
2. Integrate `bridge.status` from `renderer/hooks/useBridgeStatus.ts` for gating, banner copy, tooltip copy, and in-session deferred-work handling while offline.
3. Wire telemetry counters and ensure "Run all" bypasses network calls when offline while keeping the deferred model work ready for auto-resume.
...
- Playwright `app/tests/e2e/gui.insights.spec.ts` validates offline gating, in-session deferral, and auto-resume of model insights.
