...
- Phase 14C operator observation update on 2026-05-15: operator-tested project switch passed and floating-pane reload/rebind passed. Restore-latest ZIP-as-copy failed with visible `Project restore failed` / `Request validation failed` toast text and a trace ID after the confirmation modal correctly said a new sibling copy would be created and existing folders would not be overwritten. The preload payload mismatch has now been corrected in `app/main/preload.ts` so the restore bridge sends `restoreAsNew`, matching `services/src/blackskies/services/routers/restore.py`. This remains a restore-latest trust defect until the operator reruns the receipt and confirms the corrected contract. No `docs/auds/...` path existed, so no path merge was required. `Autofill.setAddresses` appeared in console output but is currently treated as unrelated Chromium/DevTools noise.
- Phase 14C restore-latest payload contract fix recorded on 2026-05-15. `app/main/preload.ts` now serializes `restoreAsNew` for restore-latest ZIP-as-copy requests, and `app/main/__tests__/serviceApi.test.ts` now pins the camelCase payload contract. Phase 14C remains open because the operator must rerun the restore-latest receipt before closure; Phase 15 still owns broader restore-hardening beyond this wire-contract repair.
