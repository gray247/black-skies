...
- [2026-08-23] P5-UX-01 rail-and-controls repair qualification: the focused conventional candidate passed the clean Stage 19 regression with `49` critical files / `747` passed / `2` skipped, startup preflight `1/1`, and Electron `32/32`, with no recurring viewport-stabilization warning. The replacement Windows installer was rebuilt and verified against the exact candidate; the package receipt records the installer fingerprint, receipt/commit binding, zero unpacked forbidden paths, and `NotSigned` private unsigned-internal-RC signature truth. The explicit application-only installed lifecycle passed packaged startup, optional Command transition, zero forbidden runtime processes, exact Markdown export, representative 100-Unit workflow, uninstall, external-data preservation, and same-installer reinstall. The elevated firewall-isolated lane was not run because this session is not elevated; no firewall rule was created. Classification: implemented, mechanically tested, package-qualified, and application-only installed evidence; Human Gate 2/3 acceptance, exact-installed status, Program 5 closure, and push remain unclaimed.
