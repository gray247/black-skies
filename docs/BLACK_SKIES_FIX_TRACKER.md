...
- Phase 14C operator observation update on 2026-05-15: operator-tested project switch passed and floating-pane reload/rebind passed. Restore-latest ZIP-as-copy failed with visible `Project restore failed` / `Request validation failed` toast text and a trace ID after the confirmation modal correctly said a new sibling copy would be created and existing folders would not be overwritten. The current likely root cause is a frontend/preload payload mismatch: `app/main/preload.ts` serializes `restore_as_new`, while `services/src/blackskies/services/routers/restore.py` expects `restoreAsNew`. This is now classified as a restore-latest trust defect, not a backend route validation policy change; it blocks closure until it is either fixed or explicitly deferred with operator acceptance. No `docs/auds/...` path existed, so no path merge was required. `Autofill.setAddresses` appeared in console output but is currently treated as unrelated Chromium/DevTools noise.
