...
- Package `19.19`, packaging and installation proof, is formally closed by `stage19_package_19_19_closure.md`. Package `19.20` packaged-RC manual acceptance is explicitly authorized and twice-reviewed under `stage19_package_19_20_packaged_rc_acceptance_plan.md`. Its original Package `19.19` installer is now disqualified: supported assisted launch reproduced `BS-19.20-W01` as a P1 `0xC0000005` crash in the bundled NSIS `System.dll` per-user known-folder call. A pinned narrow compatibility correction now passes local build, install, registration, and uninstall proof on the affected host; full fixed-gate and clean Windows candidate requalification remain required before human acceptance resumes. Planning-audit finding `BS-19.20-P2-01` remains closed. Packages `19.20` through `19.22` remain open, Package `19.21` is not authorized, and Package `19.22` remains the final V1.0 closure and release boundary
