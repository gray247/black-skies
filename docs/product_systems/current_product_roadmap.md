...
- Package `19.19`, packaging and installation proof, is explicitly authorized and active from clean synchronized commit `17c59f5` under `stage19_package_19_19_scope_and_inspection.md`. It may produce and qualify only the unsigned Windows 11 x64 per-user NSIS `1.0.0-rc1` candidate; Package `19.20` packaged-RC manual acceptance is not authorized. Packages `19.19` through `19.22` remain open, and Package `19.22` remains the final V1.0 closure and release boundary
