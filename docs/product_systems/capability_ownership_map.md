...
| emotional trajectory support | `Visualizer` for advisory and view state | `Author Intent / Story Setup`, `Story Unit`, `Outline`, `Character Cards`, `Narrative Insertion / Narrative Assertion`, `Writing Surface`, `Command Center`, `Theme System`, `Critique` | manuscript-observed movement projection, planned-target overlays, candidate interpretations, provider-neutral signals, and Visualizer preferences | inferred emotion is not author intent by default; emotional planning stays with its existing owner by scope; Visualizer does not require per-passage manual annotation | existing dossier plus `visualizer_program_delivery_plan.md` | shallow | exact emotional planning taxonomy and display contract remain open | overlap with critique, theme, pacing, and character support |
...
| timeline, pacing, and pressure analysis | `Timeline / Pacing / Pressure` for support state and temporary findings | `Narrative Insertion / Narrative Assertion`, `Author Intent / Story Setup`, `Outline`, `Story Unit`, `Character Cards`, `Lore Cards`, `Continuity`, `Visualizer`, `Critique` | timeline comparisons and order-basis state, pacing comparisons and overlays, bounded pressure comparisons, prototype-specific projection state, and temporary findings | not manuscript truth, accepted chronology truth, project intent, card truth, or structural authority | existing dossier | shallow | exact chronology vocabulary, pacing defaults, pressure display, and support-history depth remain open | overlap with emotion, foreshadow, continuity, and planning owners |
...
| craft analyzer family: senses, overused words, and cliche analysis | separate writer-facing analyzers `Senses Usage`, `Overused Words`, and `Cliche Detection` under shared `craft_analyzer_family_contract.md` | `Critique`, `Writing Surface`, `Command Center`, `Companion`, `Signal Architecture`, `Feedback Notes` | advisory craft findings plus project-specific analyzer support state such as exclusions, intentional-use markers, bounded history, filters, and comparison state | no truth ownership, no note ownership, no signal ownership, and no taste-as-law authority | existing dossier | shallow | exact default visibility, settings persistence, history depth, and cross-analyzer pattern surfacing remain open | multiple craft analyzers can duplicate critique functionality without the shared family contract |
...
| governed recall and retained advisory memory | `Memory Lab` | `Companion`, `Continuity`, `Critique`, `Project Index` | durable memory | memory recall is not truth and must not become shadow canon | existing dossier | mature enough for now | none | recall versus retrieval overlap with `Project Index / Search / Retrieval` |
...
| import destination classification and staging | `Import Export Document Interchange` | `Narrative Insertion / Assertion`, `Feedback Notes`, `Binder`, `File Manager` | staged import state, classification state, and transfer metadata | import does not auto-canonize or auto-place truth, and Binder only exposes staging for navigation | existing dossier | mature enough for now | none | overlap with file identity, source-owner acceptance, and staged-search visibility |
...
| Capability | Current owner | Supporting systems | Authority or state owned | Explicit non-owner boundaries | Lifecycle stage | Maturity | Unresolved Jason decision | Important overlap or duplication risk |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| test evidence and proof discipline | `Testing / Harness / Evidence Contract` | `Diagnostics`, `Service Health`, `Snapshots` | evidence expectations only | tests do not create product truth or implementation approval | existing dossier | mature enough for dossier completion work | none | validation success can be overstated as readiness |
...
| series grouping and cross-story linking | `Series Binder / Cross-Story Linking` | `Binder`, `Memory Lab`, `Character Cards`, `Lore Cards`, `Outline` | series structure, Story Chains, shared identity shells, visibility boundaries, and lineage state | series containers do not own canon | existing dossier | mature enough for now | none | overlap with binder, shared-fact owners, memory scope, and lineage review |
