...
- Feeds into: `Writing Surface`, `Command Center Surface`, `Continuity`, `Relationship Map`, `Visualizer`, `Outline`
...
Character facts follow explicit author-governed lifecycle states:

- candidate: a proposed fact, imported fact, advisory inference, or AI
  suggestion that has not yet been accepted
...
- superseded: a newer accepted fact replaces an older one while
  retaining lineage
- merge: multiple supported records combine without flattening
  conflicting history
- split: one fact or record is separated into distinct facts or records
...
Character facts may remain true for one story period and false in
another. The card should distinguish currently true, bounded-period
true, formerly true, uncertain, contradicted, and superseded posture.
Earlier truth is not an error merely because later truth exists.

Manuscript prose may support, contradict, or leave a fact uncertain, but
it does not automatically rewrite the card. Contradictions remain
advisory until the author accepts a durable fact change through this
owner.

## 15B. Primary Ownership, Secondary References, And Transfer

Character-to-character claims normally route to `Character Cards`.
...
Accepted character facts can also be referenced by other systems, but
those systems do not become duplicate authoritative records.
