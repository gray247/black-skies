...
| `Writing Surface`, `Command Center`, `Companion` | Promote advisory output to truth | target truth owner: `Narrative Insertion / Assertion`, `Lore Cards`, `Character Cards`, or parked project-truth path in `Workflow Spine` | `T2 + T6` | explicit accepted truth object | signals, notes, memory, routing state, transfer history | source trace showing candidate origin and author acceptance | leave as candidate only | yes |
