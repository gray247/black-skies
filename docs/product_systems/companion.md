...
- May `Companion` silently promote, merge, delete, resolve, or rewrite card facts? Answered: no. It may route to cards or supporting assertions, explain uncertainty, and offer suggestions, but explicit author review through the owning truth path is required for any accepted fact change.
