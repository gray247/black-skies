...
- `Visualizer` (formerly `Emotion Graph`) remains a derived support system. Project-level emotional intent belongs to `Author Intent / Story Setup`, Story Unit emotional purpose belongs to `Story Unit`, structural emotional targets belong to `Outline`, accepted character emotional state belongs to `Character Cards`, and manuscript-grounded emotional evidence belongs to accepted assertions. Its staged Program 7 foundation and Program 9 semantic work are defined in `visualizer_program_delivery_plan.md`.
- `Timeline / Pacing / Pressure` remains a derived support system with three modules. Accepted time facts belong to existing truth owners, project-level pacing intent belongs to `Author Intent / Story Setup`, structural pacing intent and prototype order belong to `Outline`, Story Unit pacing purpose belongs to `Story Unit`, and bounded pressure support remains distinct from emotional truth.
- `Senses Usage`, `Overused Words`, and `Cliche Detection` remain separate specialized advisory analyzers under the shared `Craft Analyzer Family` contract. They may own project-specific support state such as exclusions, intentional-use markers, bounded history, scope preferences, and comparison state, but they do not own manuscript truth, durable notes, or durable signals.
...
- Impossible chronology, unsupported dates, duration conflicts, pacing mismatches, pressure collapse, stale timeline support, and prototype reveal-order problems begin as temporary findings. They may be dismissed, marked intentional, saved as notes, elevated to signals, or used to revise prose, planning, or accepted time facts, but they must not silently create durable editorial artifacts.
- Craft-analyzer findings such as sensory imbalance, distracting repetition, or possible cliche begin as temporary findings. They may be dismissed, ignored, suppressed, marked intentional, saved as notes, elevated to signals, or used to revise prose, but they must not silently create durable editorial artifacts.
