...
| ARC-P3-01 | Logical surface host independent of mandatory physical-window placement | Architecture audit `ARC-P3-01` | next | Program 3 planning and implementation | One Writing authority supports a summonable single-screen Command surface and optional secondary placement without duplicated truth |
| ARC-P3-02 | Behavior-locked Stage 19 controller/view decomposition | Architecture audit `ARC-P3-02` | next | First Program 3 implementation batch | New shell views consume explicit state/actions while existing workflow state machines remain singular |
| ARC-P3-03 | Sanitized Command Center Review projection and source return path | Architecture audit `ARC-P3-03` | next | Program 3 | Role, project, generation, request, selection, stale, and owner-action boundaries are qualified |
| ARC-P3-04 | Dedicated Stage 19 preload parity across development, test, and package hosts | Architecture audit `ARC-P3-04` | next | Program 3 | Dedicated hosts expose the same narrow bridge and role restrictions in every environment |
| ARC-P3-05 | Truthful active-surface TypeScript boundary | Architecture audit `ARC-P3-05` | next | Program 3 | Focused and full typechecks both cover all current shell sources and shared contracts |
| ARC-P3-06 | Scoped Program 3 style and token boundary | Architecture audit `ARC-P3-06` | next | Program 3 | New literary shell states have one restrained token source without a global legacy CSS cleanup |
| ARC-P3-07 | Consistent presentation-state vocabulary over domain-specific errors | Architecture audit `ARC-P3-07` | next | Program 3 | Loading, advisory, stale, degraded, failed, disabled, and offline states remain truthful across surfaces |
| ARC-H1 | Feedback Note concurrent-create lost-update risk | Architecture audit `ARC-H1` | active | Program 3 Batch P3-A | Per-project serialization or equivalent concurrency contract and deterministic evidence exist before a second create surface |
...
| TST-G1 | Supported-core receipt lists TypeScript paths beside a Python-only branch percentage | Test audit `TST-G1` | active | Program 3 Batch P3-A | Receipt scope is relabeled truthfully or separately measured TypeScript changed-boundary evidence is added before a candidate claim |
| TST-P3-01 | Logical surface-host authority evidence | Test audit `TST-P3-01` | next | Program 3 | Single-screen and optional-secondary arrangements preserve one project and one Writing mutation owner |
| TST-P3-02 | Sanitized Review projection evidence | Test audit `TST-P3-02` | next | Program 3 | Command receives only approved advisory projection data and cannot mutate or expose manuscript truth |
| TST-P3-03 | Feedback Note multi-surface concurrency evidence | Test audit `TST-P3-03` and architecture `ARC-H1` | active | Program 3 Batch P3-A | Concurrent accepted creates cannot lose a note and project isolation remains intact |
| TST-P3-04 | Presentation-state and changed-shell accessibility matrix | Test audit `TST-P3-04` | next | Program 3 | Semantic, keyboard, focus-return, Focus-mode, large-text, reduced-motion, and meaningful degraded states pass |
| TST-P3-05 | Layered visual and responsive evidence | Test audit `TST-P3-05` | next | Control Point 1D and Program 3 | Approved design tokens and states support semantic viewport evidence, targeted reference images, and Human Gate 2 |
| TST-P3-06 | Development, test, build, and package authority parity | Test audit `TST-P3-06` | next | Program 3 | Dedicated preload, role restrictions, and active type boundary are executable and consistent in every supported host |
| TST-P3-07 | Versioned performance protocol for accepted surface topology | Test audit `TST-P3-07` | next | Program 3 qualification | V1 two-window baseline stays immutable and the accepted new topology receives an exact comparable baseline |
...
| SKP-04 | Strict visual snapshot lane opt-in skip | Intentional-skip inventory, fix tracker issue 43, and test audit `TST-P3-05` | next | Control Point 1D and Program 3 | Approved layered visual assertion strategy replaces or bounds the strict cross-host snapshot lane |
...
| P3-01 | Contextual product shell | [Program 3 implementation plan](program_3_contextual_product_shell_implementation_plan.md) | active | Program 3 | Jason approved the seven-batch plan, explicitly authorized bounded runtime work, and confirmed `GPT-5.6 Sol` with `xhigh` reasoning on 2026-08-10; P3-A begins after the authorization baseline is committed and pushed |
