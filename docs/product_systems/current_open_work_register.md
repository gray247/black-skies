...
| ARC-H1 | Feedback Note concurrent-create lost-update risk | Architecture audit `ARC-H1` | next | Program 3 if Command can save; otherwise first multi-surface note workflow | Per-project serialization or equivalent concurrency contract and deterministic evidence exist before a second create surface |
...
| TST-G1 | Supported-core receipt lists TypeScript paths beside a Python-only branch percentage | Test audit `TST-G1` | next | Program 3 qualification tooling | Receipt scope is relabeled truthfully or separately measured TypeScript changed-boundary evidence is added before a candidate claim |
...
| TST-P3-03 | Feedback Note multi-surface concurrency evidence | Test audit `TST-P3-03` and architecture `ARC-H1` | next | Program 3 if Command can save; otherwise first second-surface create workflow | Concurrent accepted creates cannot lose a note and project isolation remains intact |
