...
| P7-01 | Program 7 Creation, Revision, and Readiness implementation | [Program 7 charter](program_7_creation_revision_and_readiness_charter.md), [implementation plan](program_7_creation_revision_and_readiness_implementation_plan.md), [qualification protocol](program_7_qualification_protocol.md), and [visual handoff](program_7_visual_handoff.md) | implementation-authorized-wave-1-checkpoint-pushed | Program 7 later packages | Jason accepted the charter and authorized Luna-high bounded implementation/test delegation. The planning baseline remains user commit `994e558d7ede9d9dde4df1a4e268d91be20ebd43`; the documentation-only hardening is accepted, clean, and pushed at `d3c412816fae98346ca5d4f40cce7ea38d2fe5cc`. P7-0B is complete. The reviewed Wave 1 implementation checkpoint is clean-qualified and pushed at `034ccf62edc09534f8effd2697dc16f36cdb44b9`; later Program 7 packages and gates retain their named owners. |
| P7-RT | Program 7 Readiness Track: Program 6 integration, one-model local-AI pilot, and long-form corpus | [Program 7 charter](program_7_creation_revision_and_readiness_charter.md), [implementation plan](program_7_creation_revision_and_readiness_implementation_plan.md), and [`Carmilla` source manifest](../../sample_project/program_7_corpus/carmilla/README.md) | host-feasibility-proved; qualification-required | Program 7 Readiness Track | `Carmilla` is selected and its untouched 28,199-word Project Gutenberg source is downloaded with license and hash evidence. Ollama `0.13.0` is healthy on loopback, all six installed models are visible, and provisional `qwen3:4b` returned exactly `OK` and loaded on CPU. This removes the old runtime/ACL-repair blocker but does not qualify the model. The derived baseline/revised corpus and answer key now verify deterministically; quality, latency, resource, cancellation, timeout, structured-harness, protection, mandatory no-AI baseline qualification, and mandatory local-AI pilot evidence remain open. Admission requires every pilot row to pass; a bounded limitation blocks admission and records re-entry; no silent fallback is permitted |
| HG-04 | Intelligence and creation review | Execution-control plan and [Program 7 charter](program_7_creation_revision_and_readiness_charter.md) | owned-later | Human Gate 4 | The first Program 6 finding through Program 7 revision, re-evaluation, and Jason-resolution workflow passes exact mechanical qualification and long-form human review through an independently accepted no-AI path; the mandatory local-AI pilot is executed and recorded, and its path is reviewed as admitted only if every pilot row passes. An unadmitted local-AI outcome does not invalidate the no-AI path; this gate does not silently close the remaining Program 7 workflow families |
...
### Program 7 consolidated blocker and carry-forward queue — 2026-09-09

This is the current execution-facing todo list for Program 7. It deduplicates
active blockers, gate obligations, and earlier-program regression concerns
without turning historical technical-debt notes into silent Program 7 scope.

| ID | Status | Owner / resolution stage | Exit evidence | Reopening trigger or boundary |
| --- | --- | --- | --- | --- |
| `P7-W1-E1-ADV` | closed-mechanical | P7-E1 Ideation Owner | Clean checkpoint `034ccf62` includes 27/27 focused repository/IPC tests proving fail-closed adaptive validation, stable archive/restore history, duplicate-ID rejection, protected promotion rejection, bounded evidence retention, exact promotion provenance, complete successful-receipt identity, and durable request/receipt binding | Any malformed saved record is accepted, source/AI provenance is rewritten, protected content reaches promotion/inference, successful model identity is incomplete, or pinned/lineage evidence is silently discarded |
| `P7-W1-RT1A-PROJ` | closed-mechanical | RT-1A Program 6 integration | Clean checkpoint `034ccf62` excludes protected summary/evidence sentinels from the serialized production projection while retaining ordinary signals; the changed qualification slice passes | Protected source text survives the projection/preload boundary or ordinary content is incorrectly redacted |
| `P7-W1-C1-HASH` | closed-mechanical | P7-C1 Revision Candidate Owner | Clean checkpoint `034ccf62` rehashes persisted source bodies and rejects mismatched request source, usage length, or receipt prompt hash before a candidate is saved as current; 7/7 focused tests pass | A tampered source/hash or unbound local-AI response is accepted as current |
| `P7-W1-CHECKPOINT` | closed | Program 7 coordinator / Wave 1 checkpoint | Exact commit `034ccf62edc09534f8effd2697dc16f36cdb44b9`, tree `636e633953e684fcc4ac5ef74cdacf177de5182a`, passed the clean focused, regression, typecheck, lint, build, corpus, diff, and Companion mechanical gates and was pushed | Any later evidence that the recorded tree or gate result was mismatched reopens only this checkpoint |
| `P7-HG4-REG-P4` | mechanical-pass-human-pending | P7-G2 / Human Gate 4 | Clean checkpoint `034ccf62` passed production build, 135/135 Companion renderer/controller tests, and 1/1 normal-host built-Electron journey; Jason still records the real-project visual row | A failed or uncertain human result creates one bounded Program 4 regression ticket; a pass does not reopen Program 4 |
| `P7-HG4-REG-P5` | human-pending | P7-G2 / Human Gate 4 | Anchored source navigation works across unit navigation and review-surface reopen in a substantial manuscript | A failed or uncertain row creates one bounded Program 5/Human Gate regression ticket; historical receipts remain immutable |
| `P7-HG4-REG-P6` | human-pending | P7-G2 / Human Gate 4 | Detached Emotion source return preserves project/window identity and opens the correct Writing source twice | A failed or uncertain row creates one bounded Program 6 regression ticket; a pass keeps Program 6 closed |
| `P7-W2-W4` | next-after-wave-1 | Named P7 packages and serial integration owners | Revision actions, Narrative Insertion, promotion handoff, UI, host integration, recovery, topology, accessibility, and History satisfy their package gates | Owner leakage, silent mutation, stale anchors, failed recovery, or topology/accessibility failure stops the owning package only |
| `P7-RT-NOAI` | owned-later-program7 | RT-3A plus P7-G2 before Human Gate 4 | Complete Carmilla manual no-AI finding-to-resolution loop, including source return, recheck, author resolution, recovery, and performance evidence | Corpus/hash drift or an incomplete manual path blocks Human Gate 4 and Program 7 closure |
| `P7-RT-2B` | owned-later-program7 | RT-2B in Wave 5 | Execute the full real `qwen3:4b` matrix for quality, latency, resource use, cancellation, timeout, protection, and repeatability | Any failed row keeps local AI unadmitted; it does not invalidate a complete no-AI path or authorize fallback |
| `P7-DEBT-INTAKE` | active-governance | Current open-work register | Every newly reproduced issue gets one evidence-backed row naming severity, owning program/package, resolution stage, test destination, and reopening trigger; autonomously repairable items close in the same bounded owner pass | No generic “miscellaneous tech debt” bucket and no promotion from historical-only evidence without a current trigger |

Earlier debt remains visible but non-blocking unless re-triggered: `ARC-02`
owns startup/duplication/snapshot/runtime-truth defects at the changed boundary;
`ENV-01` owns exact-candidate host/ACL/preflight constraints; `ARC-CLN-01`,
`CLN-B`, and `FIN-01` own legacy and packaging professionalization; full
conversational Companion/local-LLM work requires a separate named
local-intelligence authorization; and Program 6 cross-lens unification remains
later maturity work after the Program 7 long-form evidence path. April-era
dependency, warning, CI-artifact, `uuid`, Electron/package, and worktree notes
remain historical/deferred evidence until one of those current owners records
a reproduced trigger.
...
Program 7 runtime implementation is in progress in the separately recorded
worktree and branch in `P7-01`. The reviewed Wave 1 checkpoint is clean-
qualified and pushed at `034ccf62`; the P7-E1, RT-1A, and P7-C1 hardening
blockers are mechanically closed. Later packages remain subject to their
explicit ownership and gates.
