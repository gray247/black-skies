...
From the repository root, the role-specific import command is:

```powershell
pnpm.cmd --dir app run qualification:import-scores -- `
  --run-dir '<external-run-directory>' `
  --reviewer 'reviewer-a' `
...
The bounded `private/score-import-transaction.json` journal is specific to `IMPORT_REVIEWER_SCORE`. It records only its schema, run ID, reviewer role, source and target lifecycle, accepted-score relative path and expected SHA-256, the two fixed staging paths, expected old and new manifest SHA-256 values, transaction phase, and creation time. It never records scores, flags, notes, aliases, packet or critique prose, evidence strings, reviewer identity, or credentials. The durable phases are `JOURNAL_PREPARED`, `SCORE_STAGED`, `MANIFEST_STAGED`, `SCORE_COMMITTED`, `MANIFEST_COMMITTED`, and `COMPLETE`; lock acquisition precedes validation and journal preparation. Journal and stage files are exclusively created and synced, journal phase replacement and manifest replacement are atomic, the accepted score is copied exclusively and synced, and committed score evidence is never overwritten or deleted by rollback.
