...
- Stage 19 is open and Package `19.22` is active; the prior closure receipt is historical only and cannot complete V1
...
- Package `19.22` remains active because the prior candidate lacked the required receipt-bound qualification witness. A new exact candidate, current CI, receipt, installed lifecycle, and final human PASS are mandatory; public release remains prohibited
