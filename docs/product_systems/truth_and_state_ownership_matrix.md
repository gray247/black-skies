...
| `index state` | `Project Index / Search / Retrieval` | `Writing Surface`, `Command Center`, `Companion` | index owner may build, refresh, and prune indexes; retrieval consumers may query but not redefine index ownership | durable reference state, non-canon, source-labeled, protection-aware |
| `project organization metadata` | `Binder / Project Library` | `Writing Surface`, `Command Center`, `Project Index / Search / Retrieval`, `Document Interchange` | binder owner may mutate grouping and organization metadata; organization is not truth mutation | durable navigation metadata, non-canon |
| `file and asset metadata` | `File Manager / Asset Pane` | `Writing Surface`, `Command Center`, `Document Interchange` | file metadata and preview metadata mutate here; transfer authority still belongs elsewhere | durable browse state, must preserve hidden/protected boundaries |
...
- `Memory Lab` and `Project Index / Search / Retrieval` both use indexing and retrieval language; their state ownership is now split, but dossier alignment is still needed to prevent recall versus retrieval drift.
