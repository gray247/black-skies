...
- `accepted lore truth`: accepted structured world, setting, rule, history, or canon facts stored in author-owned lore records
- `accepted character truth`: accepted structured character facts, states, relationships, or biography facts stored in author-owned character records
...
| `accepted lore truth` | accepted structured world or canon facts | `Lore Cards` or other explicit author-owned lore records | `Continuity`, `Critique`, `Companion`, `Memory Lab`, `Signal Architecture`, `Document Interchange`, `Projection`, `Outline` | explicit author create, save, accept, convert, or update action in the lore-owning path on the selected fact or facts; explicit author-approved import into lore records | import text silently becoming lore; critique or continuity silently updating lore; memory recall silently rewriting lore; manuscript wording silently overwriting accepted lore |
| `accepted character truth` | accepted structured character facts and states | `Character Cards` or other explicit author-owned character records | `Continuity`, `Critique`, `Companion`, `Memory Lab`, `Signal Architecture`, `Document Interchange`, `Projection`, `Outline` | explicit author create, save, accept, convert, or update action in the character-owning path on the selected fact or facts; explicit author-approved import into character records | imported notes silently becoming character truth; continuity auto-fix; critique auto-fix; Memory Lab recall promotion; manuscript wording silently overwriting accepted character truth |
| `accepted project truth` | explicit project goals, author intent, boundaries, story-setup decisions | `Author Intent / Story Setup` | `Workflow Spine / Author Journey`, `Companion`, `Critique`, `Memory Lab`, `Signal Architecture`, `Document Interchange`, `Routing`, `Command Center` | explicit author confirm, save, or update action in the story-setup owner path | Companion silently setting goals; critique silently changing intent; routing silently changing project boundaries; workflow suggestion silently becoming project truth |
