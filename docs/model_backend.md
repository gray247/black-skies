...
- **Telemetry hooks:** emit routing decisions, durations, and rejection reasons to `.perf/` (see `docs/performance_telemetry_policy.md`).
...
- Architecture (`docs/architecture.md`): the router sits between FastAPI services and external LLMs.
- Endpoints (`docs/endpoints.md`): `/outline/build`, `/draft/generate`, `/draft/critique`, `/batch/critique` all invoke the router; responses stay unchanged but now include `router_trace`.
- Agents (`docs/agents_and_services.md`): agents should never call models directly; they call internal services that go through the router.
- Policies (`docs/policies.md`): reference this doc for cost/privacy expectations.
- Settings (`docs/settings.md`): AI mode selection toggles router behavior.
