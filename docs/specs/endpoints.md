...
> **Phase 8 scope:** Voice-note recording/transcription routes described in `../deferred/voice_notes_transcription.md` are deferred to Phase 9; `/api/v1/voice/*` is not available in this release.
...
Schemas v1 (see `./data_model.md` / `../critique_rubric.md`):
...
## Outline
Calls assume the renderer provides the active `project_id`; users should not re-type it once a folder is open (see `../gui/gui_layouts.md`).
...
Runs critique on a unit using the rubric (see `../critique_rubric.md`). Non-destructive; suggestions are diffs to apply later.

**Heuristic output:** Each critique includes a `heuristics` object (`pov_consistency`, `goal_clarity`, `conflict_clarity`, `pacing_fit`) derived from the metadata emitted by the synthesizer and the configured heuristics lists (`./data_model.md#heuristics-overrides`).
