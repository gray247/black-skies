...
| --- | --- | --- | --- | --- | --- |
| `app/tests/e2e/utils/sampleProject.ts` | Sample-project loader for harnessed e2e tests | Harness-driven, UI-only, backend-shaped fixtures | A requested project root exists under `sample_project/<project_id>`, and if the root is not materialized yet, that root’s own verified snapshots can provide `outline.json` and `project.json` | Mostly explicit after this pass | High |
